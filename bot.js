...
// Copy your handleCommand code here without changes
// ...
// ═══════════════════════════════════════════
//          PAIR CODE FIXED FUNCTION
// ═══════════════════════════════════════════
app.post('/api/pair', async (req, res) => {
  ...
  try {
    if (!botSocket) return res.json({ success: false, message: 'Bot socket not ready. Please wait or restart.' });

    // ─── Generate pair code (Baileys v6+) ───
    const jid = phone.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
    const pairCode = await botSocket.generatePairingCode(jid);

    // Format code for display
    const formatted = pairCode.match(/.{1,4}/g)?.join('-') || pairCode;

    // Save temporarily
    currentQR = null; // QR not needed here
    console.log(`📌 Pair code for ${phone}: ${formatted}`);

    return res.json({ success: true, code: formatted, raw: pairCode });
  } catch (err) {
...
const startBot = async () => {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (settings.autoTyping && text.startsWith(settings.prefix)) await sock.sendPresenceUpdate('composing', jid);

      if (text) await handleCommand(sock, msg, text, isGroup, sender, pushName);

      if (settings.autoTyping) await sock.sendPresenceUpdate('paused', jid);
    }
  });
  ...
  sock.ev.on('group-participants.update', async ({ id, participants, action }) => {
    ...
    if (action === 'add' && settings.welcomeMessage) {
      for (const participant of participants) {
        try {
          ...
          const welcomeText = `Welcome @${participant.split('@')[0]} to *${groupMeta.subject}* 🎉`;
    ...
    if (action === 'remove' && settings.goodbyeMessage) {
      for (const participant of participants) {
        try {
          const groupMeta = await sock.groupMetadata(id);
          await sock.sendMessage(id, { text: `Goodbye @${participant.split('@')[0]} from *${groupMeta.subject}*`, mentions: [participant] });
        } catch {}
      }
    }
  });
...
app.listen(PORT, () => {
  console.log(`🌐 Nexa Bot Server running on http://localhost:${PORT}`);
  startBot(); // ✅ Only call startBot here
});
